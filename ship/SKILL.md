...
Run `DIFF_BASE=$(git merge-base origin/<base> HEAD) && git diff "$DIFF_BASE"` and `git log origin/<base>..HEAD --oneline` to understand what was implemented.
...
2. Compute the merge base and diff the working tree against it:
   ...
   This keeps uncommitted work in scope while excluding commits that landed on the base branch after this branch was created.
